# FILE: orbit_a2a_agent/task_manager.py
import logging
...
# Import directly from your original orbit files
from flow import create_agent_flow

logger = logging.getLogger(__name__)

class orbitTaskManager(InMemoryTaskManager):
    """ TaskManager implementation that runs the orbit agent. """
    ...
    async def on_send_task(self, request: SendTaskRequest) -> SendTaskResponse:
        ...
        try:
            ...
            logger.info(f"Running orbit for task {task_params.id}...")
            agent_flow.run(shared_data) # Run the flow, modifying shared_data in place
            logger.info(f"orbit completed for task {task_params.id}")
            ...
            logger.error(f"Error executing orbit for task {task_params.id}: {e}", exc_info=True)
    ...
    async def on_send_task_subscribe(
        ...
        logger.warning(f"Streaming requested for task {request.params.id}, but not supported by this orbit agent implementation.")
