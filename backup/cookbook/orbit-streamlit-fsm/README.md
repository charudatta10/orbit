# orbit Streamlit Image Generation HITL

Human-in-the-Loop (HITL) image generation application using orbit and Streamlit. Enter text prompts, generate images with OpenAI, and approve/regenerate results.
...
-   **orbit Integration:** Uses orbit `Node` and `Flow` for image generation with built-in retries
-   **Session State Management:** Streamlit session state acts as orbit's shared store
...
-   [`nodes.py`](./nodes.py): orbit `GenerateImageNode` definition
-   [`flow.py`](./flow.py): orbit `Flow` for image generation
