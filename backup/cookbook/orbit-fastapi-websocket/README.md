# orbit FastAPI WebSocket Chat

Real-time chat interface with streaming LLM responses using orbit, FastAPI, and WebSocket.
...
- **orbit Integration**: Uses orbit `AsyncNode` and `AsyncFlow` for streaming
...
- [`nodes.py`](./nodes.py): orbit `StreamingChatNode` definition
- [`flow.py`](./flow.py): orbit `AsyncFlow` for chat processing
