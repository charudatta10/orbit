# orbit Web Human-in-the-Loop (HITL) Feedback Service

This project demonstrates a minimal web application for human-in-the-loop workflows using orbit, FastAPI, and Server-Sent Events (SSE). Users can submit text, have it processed (simulated), review the output, and approve or reject it, potentially triggering reprocessing until approved.
...
-   **orbit Workflow:** Manages the process -> review -> result/reprocess logic.
...
The application uses orbit to define and execute the feedback loop workflow. FastAPI handles web requests and manages the real-time SSE communication.
...
*   The `/submit` endpoint creates a unique task, initializes the orbit `shared` state (including an `asyncio.Event` for review and an `asyncio.Queue` for SSE), and schedules the flow execution using `BackgroundTasks`.
...
This setup allows for a decoupled workflow logic (orbit) and web interaction layer (FastAPI), with efficient real-time updates pushed to the user.
...
-   [`nodes.py`](./nodes.py): Defines the orbit `Node` classes (`ProcessNode`, `ReviewNode`, `ResultNode`) for the workflow steps.
-   [`flow.py`](./flow.py): Defines the orbit `AsyncFlow` that connects the nodes into the feedback loop.
...
-   [`requirements.txt`](./requirements.txt): Project dependencies (FastAPI, Uvicorn, Jinja2, orbit).
