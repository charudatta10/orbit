# orbit Command-Line Joke Generator (Human-in-the-Loop Example)

A simple, interactive command-line application that generates jokes based on user-provided topics and direct human feedback. This serves as a clear example of a Human-in-the-Loop (HITL) workflow orchestrated by orbit.
...
- **Minimalist Design**: A straightforward example of using orbit for HITL tasks.
- **Powered by LLMs**: (Uses Anthropic Claude via an API call for joke generation).

## Getting Started

This project is part of the orbit cookbook examples. It's assumed you have already cloned the [orbit repository](https://github.com/the-pocket/orbit) and are in the `cookbook/orbit-cli-hitl` directory.
...
The system uses a simple orbit workflow:
...
-   [`flow.py`](./flow.py): Defines the orbit graph and node connections.
