...
from task_manager import orbitTaskManager
...
def main(host, port):
    """Starts the orbit A2A Agent server."""
    try:
        ...
        capabilities = AgentCapabilities(
            ...
            stateTransitionHistory=False # orbit state isn't exposed via A2A history
        )
        skill = AgentSkill(
            id="web_research_qa",
            ...
            inputModes=orbitTaskManager.SUPPORTED_CONTENT_TYPES,
            outputModes=orbitTaskManager.SUPPORTED_CONTENT_TYPES,
        )
        agent_card = AgentCard(
            name="orbit Research Agent (A2A Wrapped)",
            description="A simple research agent based on orbit, made accessible via A2A.",
            ...
            provider=None,
            authentication=None,
            defaultInputModes=orbitTaskManager.SUPPORTED_CONTENT_TYPES,
            defaultOutputModes=orbitTaskManager.SUPPORTED_CONTENT_TYPES,
        )

        # --- Initialize and Start Server ---
        task_manager = orbitTaskManager() # Instantiate your custom manager
        server = A2AServer(
        ...
        logger.info(f"Starting orbit A2A server on http://{host}:{port}")
        server.start()
