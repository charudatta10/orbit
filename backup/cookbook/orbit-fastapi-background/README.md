# orbit FastAPI Background Jobs with Real-time Progress

A web application demonstrating orbit workflows running as FastAPI background jobs with real-time progress updates via Server-Sent Events (SSE).
...
- **orbit Integration**: Three-node workflow (Outline → Content → Style)
...
The application uses orbit to define a three-step article generation workflow. FastAPI handles web requests and manages real-time SSE communication for progress updates.
...
- [`flow.py`](./flow.py): orbit workflow definition connecting the three nodes
