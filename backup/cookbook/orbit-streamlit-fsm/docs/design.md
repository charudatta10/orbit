# Design Doc: orbit Streamlit Image Generation HITL

> Human-in-the-Loop image generation application using orbit and Streamlit
...
- 🔶 **Orange rectangles**: orbit processing nodes (GenerateImage)
...
**Using Streamlit Session State as Shared Store**: We use `st.session_state` directly as the shared store for orbit, eliminating the need for separate data structures.
